<template>
  <el-dialog
    :title="title"
    :visible.sync="seqDialogFormVisible"
    width="800px"
    @close="close"
  >
    <el-form
    ...
    >
      <el-form-item label="接口名称" prop="name">
        <el-input v-model.trim="seq_form.name" autocomplete="off"></el-input>
      </el-form-item>
      <el-form-item label="接口域名" prop="domain">
        <el-input v-model.trim="seq_form.domain" autocomplete="off"></el-input>
      </el-form-item>
      <el-form-item label="成功返回码" prop="success_code">
        <el-input
          v-model.trim="seq_form.success_code"
          autocomplete="off"
        ></el-input>
      </el-form-item>
      <el-form-item label="失败返回码" prop="error_code">
        <el-input
          v-model.trim="seq_form.error_code"
          autocomplete="off"
        ></el-input>
      </el-form-item>
      <el-form-item label="启动任务URI" prop="start_uri">
        <el-input
          v-model.trim="seq_form.start_uri"
          autocomplete="off"
        ></el-input>
      </el-form-item>
      <el-form-item label="停止任务URI" prop="stop_uri">
        <el-input
          v-model.trim="seq_form.stop_uri"
          autocomplete="off"
        ></el-input>
      </el-form-item>
      <el-form-item label="重置任务URI" prop="reset_uri">
        <el-input
          v-model.trim="seq_form.reset_uri"
          autocomplete="off"
        ></el-input>
      </el-form-item>
      <el-form-item label="查询任务状态URI" prop="status_uri">
        <el-input
          v-model.trim="seq_form.status_uri"
          autocomplete="off"
        ></el-input>
      </el-form-item>
      <el-form-item label="获取任务结果URI" prop="result_uri">
        <el-input
          v-model.trim="seq_form.result_uri"
          autocomplete="off"
        ></el-input>
      </el-form-item>
      <el-form-item label="获取运行日志URI" prop="log_uri">
        <el-input v-model.trim="seq_form.log_uri" autocomplete="off"></el-input>
      </el-form-item>
      <el-form-item label="是否启用该配置" prop="is_active">
        <el-checkbox v-model.trim="seq_form.is_active"></el-checkbox>
      </el-form-item>
      <el-form-item label="项目描述/备注" prop="remark">
        <el-input
          v-model.trim="seq_form.remark"
          type="textarea"
...
<script>
import {
  createSequencingTaskInterface,
  updateSequencingTaskInterface,
  retrieveSequencingTaskInterface,
} from "@/api/task_interface";

export default {
  name: "SeqTaskInterfaceEdit",
  data() {
    return {
      seq_form: {
        id: "",
        name: "假设接口",
        domain: "http://127.0.0.1:8000/sequencing/",
        success_code: "200 ",
        error_code: "400",
        start_uri: "start",
        stop_uri: "stop",
  ...
  methods: {
    showEdit(row) {
      ...
      this.seqDialogFormVisible = true;
    },
    async detail(instance_id) {
      const data = await retrieveSequencingTaskInterface(instance_id);
      const results = data.results;
      this.seq_form = Object.assign(
        {},
        {
          id: results.id,
          name: results.name,
    ...
    close() {
      this.$refs["seq_form"].resetFields();
      this.seq_form = this.$options.data().seq_form;
      this.seqDialogFormVisible = false;
      this.$emit("fetchData");
    },
    save() {
      this.$refs["seq_form"].validate(async (valid) => {
        if (valid) {
          let data = {};
          if (this.edit_type === "create") {
            data = await createSequencingTaskInterface(this.seq_form);
          } else {
            data = await updateSequencingTaskInterface(
              this.seq_form.id,
              this.seq_form
            );
          }
