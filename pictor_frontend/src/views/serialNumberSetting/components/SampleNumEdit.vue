<template>
  <el-dialog
    :title="title"
    :visible.sync="sampleNumFormVisible"
    width="800px"
    @close="close"
  >
...
<script>
import {
  createSampleSerialNumberSetting,
  updateSampleSerialNumberSetting,
  retrieveSampleSerialNumberSetting,
...
export default {
  ...
  methods: {
    showEdit(row) {
      ...
      this.sampleNumFormVisible = true;
    },
    async detail(instance_id) {
      const data = await retrieveSampleSerialNumberSetting(instance_id);
    ...
    close() {
      ...
      this.sampleNumFormVisible = false;
    },
    save() {
      this.$refs["form"].validate(async (valid) => {
        if (valid) {
          ...
          if (this.edit_type === "create") {
            data = await createSampleSerialNumberSetting(this.form);
          } else {
            data = await updateSampleSerialNumberSetting(
              this.form.id,
              this.form
            );
          }
