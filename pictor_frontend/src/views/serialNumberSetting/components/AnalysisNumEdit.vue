<template>
  <el-dialog
    :title="title"
    :visible.sync="analysisNumFormVisible"
    width="800px"
    @close="close"
  >
...
<script>
import {
  createAnalysisSerialNumberSetting,
  updateAnalysisSerialNumberSetting,
  retrieveAnalysisSerialNumberSetting,
} from "@/api/serialNumberSetting";

export default {
  name: "AnalysisNumEdit",
  data() {
    return {
      form: {
        id: "",
  ...
  methods: {
    showEdit(row) {
      if (!row) {
        this.title = "新增分析任务编号规则";
        this.edit_type = "create";
      } else {
        this.title = "编辑分析任务编号规则";
        this.edit_type = "update";
        this.detail(row.id);
      }
      this.analysisNumFormVisible = true;
    },
    async detail(instance_id) {
      const data = await retrieveAnalysisSerialNumberSetting(instance_id);
    ...
    close() {
      ...
      this.analysisNumFormVisible = false;
    },
    save() {
      this.$refs["form"].validate(async (valid) => {
        if (valid) {
          ...
          if (this.edit_type === "create") {
            data = await createAnalysisSerialNumberSetting(this.form);
          } else {
            data = await updateAnalysisSerialNumberSetting(
              this.form.id,
              this.form
            );
          }
